...
from opentelemetry.instrumentation.dify.utils import TRACELOOP_TRACE_CONTENT
...
def instrument_legacy(reader, tracer_provider, meter_provider):
    dify_instrumentor = DifyInstrumentor()
    dify_instrumentor.instrument(
        tracer_provider=tracer_provider, meter_provider=meter_provider
    )

    yield

    dify_instrumentor.uninstrument()
...
def environment():
    if not os.environ.get("DIFY_API_KEY"):
        os.environ["DIFY_API_KEY"] = "app-test-api-key"
